...
export const fetchProductById = async (id: string): Promise<Product | null> => {
  try {
    const res = await fetch(`https://market-place-builder-hacakathon.vercel.app/api/products/${id}`);
...
export const fetchCategories = async (): Promise<string[]> => {
  const res = await fetch("https://market-place-builder-hacakathon.vercel.app/api/category");
